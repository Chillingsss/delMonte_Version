...
const SecuritySettingsModal = ({ onClose }) => {
	...
	const fetchSettings = async () => {
		try {
			...
			if (response.data && !response.data.error) {
				const newEverylogs = response.data.setting_everylogs === 1;
				const newDays = response.data.setting_days;
				setSettingEverylogs(newEverylogs);
				setSettingDays(newDays);
				setInitialSettings({
					everylogs: newEverylogs,
					days: newDays,
				});
			}
		} catch (error) {
			console.error(
				"Error fetching 2FA settings:",
	...
	useEffect(() => {
		const hasChanges =
			settingEverylogs !== initialSettings.everylogs ||
			settingDays !== initialSettings.days;

		const isValid = !loading && hasChanges;

		setFormValid(isValid);
	}, [settingEverylogs, settingDays, initialSettings, loading]);
				...
				<div className="flex flex-col h-full">
										...
										{!settingEverylogs && settingDays === null && (
											<p className="text-sm text-muted-foreground mt-3 pl-7 flex items-start gap-1.5">
												...
												<span>
													...
													or enable 2FA for every login to activate it.
												</span>
											</p>
										)}
										{!settingEverylogs && settingDays !== null && (
											<p className="text-sm text-amber-600 dark:text-amber-400 mt-3 pl-7 flex items-start gap-1.5 bg-amber-500/10 dark:bg-amber-500/5 p-2 rounded-md border border-amber-500/20 dark:border-amber-500/10">
												...
												<span>
													...
													{settingDays} {settingDays === 1 ? "day" : "days"}.
												</span>
											</p>
										)}
									</div>
									...
									<Alert className="bg-secondary/50 dark:bg-secondary/30 border-secondary transition-all duration-300 hover:bg-secondary/70 dark:hover:bg-secondary/40">
										<AlertDescription className="text-sm flex gap-2">
											...
											<div>
												...
												needs. Leaving both options unselected will disable 2FA
												completely.
											</div>
										</AlertDescription>
									</Alert>
					...
					<div className="mt-6 space-y-3">
						<Button
							...
							title={
								!formValid
									? "Please make changes and fill in all required fields"
									: ""
							}
						>
							{loading ? (
