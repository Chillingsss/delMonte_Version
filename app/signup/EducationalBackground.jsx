...
function EducationalBackground({
  ...
  const handleCloseAlert = (status) => {
    if (status === 1) {
      const filteredEducationDatas = educationDatas.filter(
        (_, index) => index !== indexToRemove
      );
      setEducationDatas(filteredEducationDatas);
      storeDataInSession(
        "educationalBackground",
        JSON.stringify(filteredEducationDatas)
      );
    }
    setShowAlert(false);
  };
  ...
  const handleCloseCourseModal = (status) => {
    if (status !== 0) {
      ...
      storeDataInSession(
        "educationalBackground",
        JSON.stringify([...educationDatas, status])
      );
    }
  ...
  useEffect(() => {
    if (
      getDataFromSession("educationalBackground") !== null ||
      getDataFromSession("educationalBackground") !== "[]"
    ) {
  ...
  return (
    <div>
              ...
              <Alert key={index} className="relative w-full bg-[#0e5a35] mt-3">
                ...
                <AlertTitle className="text-md ">
                  <div className="mb-3">
                    Institution:{" "}
                    {
                      institutionList.find(
                        (item) => item.value === data.institution
                      )?.label
                    }
                  </div>
                  ...
                  <div className="mb-3">
                    Course:{" "}
                    {
                      courseList.find((item) => item.value === data.course)
                        ?.label
                    }
                  </div>
                  <div className="mb-3">
                    Date Graduated: {formatDate(data.courseDateGraduated)}
                  </div>
                </AlertTitle>
              </Alert>
            ))}
      ...
      <AddCourseModal
        ...
        graduateCourseList={graduateCourseList}
