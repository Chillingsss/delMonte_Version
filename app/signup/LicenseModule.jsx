...
function LicenseModule({ licenseType, licenseList }) {
  ...
  const handleCloseAlert = (status) => {
    if (status === 1) {
      ...
      storeDataInSession("licenses", JSON.stringify(filteredData));
    }
    setShowAlert(false);
  };
  ...
  const handleCloseModal = (status) => {
    if (status !== 0) {
      ...
      setData(newData);
      storeDataInSession("licenses", JSON.stringify(newData));
    }
    setShowModal(false);
  };
  ...
  useEffect(() => {
    const savedData = getDataFromSession("licenses");
              ...
              {data.map((datas, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    {
                      licenseType.find(
                        (item) => item.value === datas.licenseType
                      )?.label
                    }
                  </TableCell>
                  <TableCell>
                    {
                      licenseList.find((item) => item.value === datas.license)
                        ?.label
                    }
                  </TableCell>
